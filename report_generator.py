...
class CVReportGenerator:
    ...
    def _create_score_bar(self, score: int, max_width: float = 3.0) -> Table:
        """Create a visual score bar as a table"""
        normalized_score = min(max(score, 0), 100)
        filled_width = (normalized_score / 100) * max_width
        ...
        if normalized_score >= 75:
            ...
            bar_color = HexColor("#F59E0B")  # Amber
        else:
            bar_color = self.warning_color
        
        if filled_width > 0:
            bar_data[0].append(Paragraph(f"<b>{normalized_score}%</b>", 
                                         ParagraphStyle(
                                             name='ScoreText',
                                             fontSize=9,
        ...
        bar_table = Table(bar_data, colWidths=[filled_width * inch, empty_width * inch])
        bar_table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (0, 0), bar_color),
            ('BACKGROUND', (1, 0), (1, 0), HexColor("#E5E7EB")),
            ('HEIGHT', (0, 0), (-1, -1), 0.2 * inch),
